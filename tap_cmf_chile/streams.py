...
class UTMStream(CMFChileAPIV3Stream):
    """Define custom stream."""

    name = "UTM"
    path = "/utm/posteriores/{year}/{month}"
    primary_keys: t.ClassVar[list[str]] = ["Fecha"]
    replication_key = "Fecha"
    is_sorted = True
    schema_filepath = SCHEMAS_DIR / "utm.json"  # noqa: ERA001
    records_jsonpath = "$.UTMs[*]"
...
class DolarStream(CMFChileAPIV3Stream):
    """Define custom stream."""

    name = "DOLAR"
    path = "/dolar/posteriores/{year}/{month}/dias/{day}"
    primary_keys: t.ClassVar[list[str]] = ["Fecha"]
    replication_key = "Fecha"
    is_sorted = True
    schema_filepath = SCHEMAS_DIR / "dolar.json"  # noqa: ERA001
    records_jsonpath = "$.Dolares[*]"
